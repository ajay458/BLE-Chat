...
- (void) chatInputNewMessageSent:(NSString *)messageString {
    ...
    chatInfo.message = messageString;
    chatInfo.isReciever = NO;
    chatInfo.messagedate = [NSDate date];

    if ([(NSObject *)_delegate respondsToSelector:@selector(chatController:didSendMessage:)]) {
        [_delegate chatController:self didSendMessage:chatInfo];
    }
    else {
...
- (CGSize)collectionView:(UICollectionView *)collectionView
  ...
  sizeForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    ChatInfo * chatInfo = _messagesArray[[indexPath indexAtPosition:1]];

    static int offset = 20;

    if (!chatInfo.messageSize) {
        NSString * content = chatInfo.message;
        ...
        chatInfo.messageSize = [NSValue valueWithCGSize:rect.size];

        return CGSizeMake(width(_myCollectionView), rect.size.height + offset);
    }
    else {
        return CGSizeMake(_myCollectionView.bounds.size.width, [chatInfo.messageSize CGSizeValue].height + offset);
    }
}
